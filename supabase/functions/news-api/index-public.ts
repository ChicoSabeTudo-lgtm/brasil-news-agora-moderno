// ============================================
// NEWS API - Versão Pública (Sem Autenticação Obrigatória)
// ============================================
// Esta versão permite acesso público sem necessidade de chaves de API
// Ideal para uso em n8n, Zapier, e outras integrações
// ============================================
...
serve(async (req) => {
  ...
  try {
    ...
    // GET - Buscar notícias (ACESSO PÚBLICO)
    if (req.method === 'GET') {
    ...
    // POST - Criar notícia (REQUER AUTENTICAÇÃO)
    if (req.method === 'POST') {
      // Para POST, ainda exigimos autenticação
      const authHeader = req.headers.get('authorization');
      const apiKey = req.headers.get('apikey');
      
      if (!authHeader && !apiKey) {
        return new Response(
          JSON.stringify({ error: 'Autenticação necessária para criar notícias' }),
      ...
      if (body.images && body.images.length > 0) {
        const imageResult = await saveNewsImages(newsData.id, body.images, supabaseAdmin);
    ...
    return new Response(
      JSON.stringify({
        ...
        message: 'Use GET para buscar (público) ou POST para criar notícias (autenticado)',
      }),
