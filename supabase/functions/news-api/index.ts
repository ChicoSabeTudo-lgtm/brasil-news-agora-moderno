...
serve(async (req) => {
  ...
  try {
    ...
    const SUPABASE_ANON_KEY =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNwZ3VzanJqcmhmeWNoaGR3aXhuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE4MTMwODAsImV4cCI6MjA2NzM4OTA4MH0.cjsRonxr7utjcpQoyuYCUddCQR9C60YNQqyxj07atsE";
    ...
    if (req.method === "GET") {
      ...
      try {
        ...
        try {
          const categoryIds = [
            ...new Set(
              newsData.map((item) => item.category_id).filter(Boolean),
            ),
          ];
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (!body.author_id || typeof body.author_id !== "string") {
          errors.push({
            field: "author_id",
            message: "ID do autor é obrigatório",
          });
        }
        ...
        const referenceDate = body.published_at
        ...
        while (true) {
          ...
          if (slugCheckError) {
            console.warn(
              "⚠️ Erro ao verificar slug existente:",
              slugCheckError,
            );
            break;
          }
        ...
        const publishImmediately = toBoolean(body.publish_immediately);
        const wantsFeatured = toBoolean(body.is_featured);
        ...
        const { data: newsData, error: newsError } = await supabaseAdmin
          ...
          .insert({
            ...
            meta_description:
              body.meta_description || body.subtitle || body.title,
        ...
        if (body.images && body.images.length > 0) {
          ...
          for (let i = 0; i < body.images.length; i++) {
            ...
            try {
              const base64Clean = image.base64.replace(
                /^data:image\/\w+;base64,/,
                "",
              );
              const imageBuffer = Uint8Array.from(atob(base64Clean), (c) =>
                c.charCodeAt(0),
              );

              const contentType = image.base64.startsWith("data:image/png")
                ? "image/png"
                : image.base64.startsWith("data:image/gif")
                  ? "image/gif"
                  : image.base64.startsWith("data:image/webp")
                    ? "image/webp"
                    : "image/jpeg";
              ...
              await supabaseAdmin.from("news_images").insert({
                news_id: newsData.id,
                image_url: publicUrl,
        ...
        console.log(
          "🎉 POST - Notícia criada com sucesso",
          publishImmediately ? "(PUBLICADA)" : "(RASCUNHO)",
        );
