...
export default function NewClientModal({ open, onOpenChange }: Props) {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      await createContact(form.name, 'cliente', {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            <Label className="flex items-center gap-2"><User className="w-4 h-4" /> Nome do Cliente *</Label>
            <Input placeholder="Nome completo ou razão social" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </div>
          <div>
          ...
          <div>
            <Label className="flex items-center gap-2"><User className="w-4 h-4" /> Pessoa de Contato</Label>
        ...
        <DialogFooter className="mt-2">
          ...
          <Button onClick={handleCreate} disabled={!canSave}>Criar Cliente</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
